...
import { 
  ...
  UtensilsCrossed, Navigation, X, ChevronRight, CheckCircle2 
} from 'lucide-react';
...
const MyOrders: React.FC = () => {
  const { orders, loadOrders } = useApp(); // Get loadOrders from context
  const [activeTab, setActiveTab] = useState<'Active' | 'Past'>('Active');
  const [selectedId, setSelectedId] = useState<string | null>(null);

  // --- 1. AUTO-REFRESH LOGIC ---
  useEffect(() => {
    // Poll for updates every 5 seconds so status changes (like Ready -> Claimed) show up live
  ...
  const filteredOrders = orders.filter(o => {
    // "Claimed" means the student picked it up, so it goes to Past.
  ...
  const renderStatusBadge = (status: string) => {
    ...
    if (status === 'Reserved' || status === 'Payment Pending') { // Backend maps PAID -> Reserved
      return (
              ...
              <div className="overflow-y-auto p-6 scrollbar-hide">
                ...
                <div className="mb-6">
                  ...
                  >
                    ...
                    {(selectedOrder.status === 'Claimed' || selectedOrder.status === 'Completed') && (
                ...
                {/* QR Code / Pickup Code Section (Only for Active Orders) */}
                {(selectedOrder.status === 'Reserved' || selectedOrder.status === 'Ready') && (
                    <motion.div 
                    ...
                    <div>
                        ...
                        {/* NOTE: 'qrCode' in your user.py is actually the 'pickup_code' or 'visible_code'.
                            We display it here for the student to show the staff.
                        */}
